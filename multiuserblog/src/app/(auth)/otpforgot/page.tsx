...
function OtpContent() {
  ...
  // Fallback in case query string is passed directly as ?user@example.com
  if (!email) {
    const rawSearch = searchParams.toString();
    if (rawSearch) {
      const decoded = decodeURIComponent(rawSearch).replace(/=$/, '');
      if (decoded.includes('@')) {
        email = decoded;
      }
    }
  ...
  useEffect(() => {
    ...
    const sendMail = async () => {
      try {
        ...
        setError(err.response?.data?.message || 'Error sending initial OTP email');
      }
    };
    sendMail();
  }, [email]);
